...
inline __device__ void apply_token_rotary_embedding(
  ...
  if (IS_NEOX) {
    ...
    // cos/sin have shape [..., rotary_dim/2], each index corresponds to one (x,y) pair.
    x_index = 2 * rot_offset;
    y_index = x_index + 1;

    // do math in float32, cast back to scalar_t.
    ...
    // GPT-J style / LLaMA style, matching Python when cos/sin are [..., head_size]
    // Here embed_dim is the "half" dimension; cos/sin are length 2 * embed_dim.
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      "rotary_embedding",
      &rotary_embedding,
      "Apply RoPE with separate cos/sin tensors (FlashInfer-style kernel)",
      py::arg("cos"),
      py::arg("sin"),
      py::arg("query"),
      py::arg("key"),
