/*
 * Standalone rotary embedding kernel for quick testing
 * 
 * Compile: nvcc -std=c++17 -O3 --shared -Xcompiler -fPIC \
...
// Simple macro for scalar type dispatch
...
inline __device__ void apply_rotary_embedding(
    ...
    const scalar_t* __restrict__ current_token_cos_ptr,
    const scalar_t* __restrict__ current_token_sin_ptr,
    const int head_size,
    const int num_heads,
