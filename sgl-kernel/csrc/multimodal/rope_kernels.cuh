...
get_elem_offset_impl(uint32_t idx, uint32_t head_idx, uint32_t feat_idx, size_t stride_n, size_t stride_h) {
...
#pragma unroll
    for (uint32_t i = 0; i < vec_size; ++i) {
      ...
      vec[i] =
          vec[i] * cos_val + ((threadIdx.x * vec_size < rotary_dim / 2) ? -permuted_vec[i] : permuted_vec[i]) * sin_val;
    }
  }
  return vec;
}
...
template <bool interleave, uint32_t vec_size, uint32_t bdx, typename DType, typename IdType, typename QuantType>
__global__ void RopeQuantizeKernel(
    DType* q_rope_in,
    DType* k_rope_in,
    DType* q_nope_in,
    DType* k_nope_in,
    QuantType* q_rope_out,
    QuantType* k_rope_out,
    QuantType* q_nope_out,
    QuantType* k_nope_out,
    float* __restrict__ cos_sin_cache,
    IdType* __restrict__ pos_ids,
    uint32_t nnz,
    uint32_t num_qo_heads,
    ...
    size_t k_rope_out_stride,
    size_t k_rope_out_stride_h,
    size_t k_nope_out_stride,
    size_t k_nope_out_stride_h,
    float quant_scale_q,
  ...
  if (bx * bdy + ty < nnz) {
    ...
    if (by < q_rope_end) {
      ...
      DType* q_rope_in_ptr =
          q_rope_in + get_elem_offset_impl(idx, q_head_idx, 0, q_rope_in_stride_n, q_rope_in_stride_h);
      QuantType* q_rope_out_ptr =
          q_rope_out + get_elem_offset_impl(idx, q_head_idx, 0, q_rope_out_stride_n, q_rope_out_stride_h);

      vec_t<float, vec_size> q_rope_vec;
      if constexpr (interleave) {
        q_rope_vec =
            vec_apply_llama_rope_cos_sin_interleave_reuse_half<vec_size, bdx>(q_rope_in_ptr, cos, sin, rope_dim);
      } else {
...
#pragma unroll
      ...
      QuantType* k_rope_out_ptr =
          k_rope_out + get_elem_offset_impl(idx, k_head_idx, 0, k_rope_out_stride, k_rope_out_stride_h);

      vec_t<float, vec_size> k_rope_vec;
      if constexpr (interleave) {
        k_rope_vec =
            vec_apply_llama_rope_cos_sin_interleave_reuse_half<vec_size, bdx>(k_rope_in_ptr, cos, sin, rope_dim);
      } else {
...
#pragma unroll
      ...
      DType* k_nope_in_ptr =
          k_nope_in + get_elem_offset_impl(idx, k_head_idx, elem_offset, k_nope_in_stride, k_nope_in_stride_h);
      QuantType* k_nope_out_ptr =
          k_nope_out + get_elem_offset_impl(idx, k_head_idx, elem_offset, k_nope_out_stride, k_nope_out_stride_h);
...
#pragma unroll
      ...
      DType* q_nope_in_ptr =
          q_nope_in + get_elem_offset_impl(idx, q_head_idx, elem_offset, q_nope_in_stride_n, q_nope_in_stride_h);
      QuantType* q_nope_out_ptr =
          q_nope_out + get_elem_offset_impl(idx, q_head_idx, elem_offset, q_nope_out_stride_n, q_nope_out_stride_h);
...
cudaError_t BatchQKApplyRotaryPosIds(
  ...
  if (interleave) {
    if (head_dim == 64)
      launch_pos_ids_kernel.template operator()<true, 64>();
    else if (head_dim == 256)
      launch_pos_ids_kernel.template operator()<true, 256>();
    else
      launch_pos_ids_kernel.template operator()<true, 128>();
  } else {
    if (head_dim == 64)
      launch_pos_ids_kernel.template operator()<false, 64>();
    else if (head_dim == 256)
      launch_pos_ids_kernel.template operator()<false, 256>();
    else
      launch_pos_ids_kernel.template operator()<false, 128>();
  }
...
cudaError_t BatchQKApplyRotary(
  ...
  if (interleave) {
    if (head_dim == 64)
      launch_rotary_kernel.template operator()<true, 64>();
    else if (head_dim == 256)
      launch_rotary_kernel.template operator()<true, 256>();
    else
      launch_rotary_kernel.template operator()<true, 128>();
  } else {
    if (head_dim == 64)
      launch_rotary_kernel.template operator()<false, 64>();
    else if (head_dim == 256)
      launch_rotary_kernel.template operator()<false, 256>();
    else
      launch_rotary_kernel.template operator()<false, 128>();
  }
